...
function nextFocus(row, index, e) {
  const length = columnRefs.value.length
...
<template>
  <el-card>
        ...
        <el-table ref="dragTableRef" v-loading="loading" :data="columns" row-key="columnId" min-height="80px" :max-height="tableHeight">
          ...
          <el-table-column label="字段描述" fixed width="120">
            <template #default="scope">
              <el-input :ref="setColumnsRef" v-model="scope.row.columnComment" @keydown="nextFocus(scope.row, scope.$index, $event)" />
            </template>
          </el-table-column>
          ...
          <el-table-column label="C#属性" width="100">
            <template #default="scope">
              <el-input v-model="scope.row.csharpField" />
            </template>
          </el-table-column>
          <el-table-column v-if="info.tplCategory != 'select'" label="插入" width="60" align="center">
            <template #default="scope">
              <el-checkbox v-model="scope.row.isInsert" :disabled="scope.row.isIncrement" />
            </template>
          </el-table-column>
          <el-table-column v-if="info.tplCategory != 'select'" label="编辑" width="60" align="center">
            <template #default="scope">
              <el-checkbox v-model="scope.row.isEdit" :disabled="scope.row.isPk || scope.row.isIncrement" />
            </template>
          </el-table-column>
          ...
          <el-table-column label="查询方式" width="90" align="center">
            <template #default="scope">
              <el-select v-if="scope.row.isQuery" v-model="scope.row.queryType" :disabled="scope.row.htmlType == 'datetime'">
    ...
    <footer class="mt20" style="text-align: center">
      ...
      <el-button type="success" icon="refresh" @click="handleQuery()">
        刷新
      </el-button>
      <el-button icon="back" @click="close()">
        返回
      </el-button>
    </footer>
  </el-card>
