...
import { delSysfile, getSysfile, listSysfile } from '@/api/tool/file.js'
// 选中id数组
const ids = ref([])
// 非单个禁用
...
<template>
  <div class="app-container">
    ...
    <el-form v-show="showSearch" ref="queryForm" :model="queryParams" label-position="left" inline @submit.prevent>
      ...
      <el-form-item label="" prop="storeType">
        <el-select v-model="queryParams.storeType" placeholder="请选择存储类型" clearable="">
          <el-option v-for="item in storeTypeOptions" :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue" />
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button v-hasPermi="['tool:file:add']" type="primary" plain icon="upload" @click="handleAdd">
          {{ $t('btn.upload') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button v-hasPermi="['tool:file:delete']" type="danger" :disabled="multiple" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
    ...
    <!-- 数据区域 -->
    <el-table ref="table" v-loading="loading" :data="dataList" border highlight-current-row @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="fileName" label="文件名" align="left" width="180" :show-overflow-tooltip="true">
        <template #default="scope">
          <el-link type="primary" :href="scope.row.accessUrl" target="_blank">
            {{ scope.row.fileName }}
          </el-link>
        </template>
      </el-table-column>
    ...
    <pagination v-model:page="queryParams.pageNum" v-model:limit="queryParams.pageSize" background :total="total" @pagination="getList" />

    <!-- 添加或修改文件存储对话框 -->
    <el-dialog v-model="open" :title="title" :lock-scroll="false" width="400px" draggable>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px" label-position="left">
        <el-row>
          ...
          <el-col v-if="form.fileNameType == 2" :lg="24">
    ...
    <!-- 添加或修改文件存储对话框 -->
    <el-dialog v-model="openView" title="查看" :lock-scroll="false">
      <el-form ref="form" :model="formView" :rules="rules" label-width="100px">
        <el-row>
          ...
          <el-col v-if="['.png', '.jpg', '.jpeg'].includes(formView.fileExt)" :lg="24">
            <el-form-item label="预览">
              <el-image :src="formView.accessUrl" fit="contain" style="width: 100px" />
            </el-form-item>
          </el-col>
          <el-col :lg="24">
