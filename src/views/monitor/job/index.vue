...
import { createTasks, deleteTasks, exportTasks, getTasks, queryTasks, runTasks, startTasks, stopTasks, updateTasks } from '@/api/monitor/job'
...
<template>
  <div class="app-container">
    ...
    <el-row class="mb8" :gutter="20">
      ...
      <el-col :span="1.5">
        <el-button v-hasPermi="['monitor:job:query']" plain type="info" icon="operation" @click="onJobLogView()">
          {{ $t('btn.log') }}
        </el-button>
      </el-col>
      <right-toolbar :show-search="searchToggle" @queryTable="handleQuery" />
    </el-row>
    <el-row>
      <el-table ref="tasks" v-loading="loading" :data="dataTasks" border="" row-key="id" @sort-change="handleSortable">
        ...
        <el-table-column sortable prop="isStart" align="center" label="任务状态" width="100">
          <template #default="scope">
            <dict-tag :value="scope.row.isStart" :options="isStartOptions" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="250" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-link v-hasPermi="['monitor:job:query']" size="small" class="ml10" icon="view" @click="handleDetails(scope.row)">
              {{ $t('btn.details') }}
            </el-link>
            <el-link v-hasPermi="['monitor:job:query']" size="small" class="ml10" icon="view" @click="handleJobLog(scope.row)">
              {{ $t('btn.log') }}
            </el-link>
            <el-link
    ...
    <el-dialog v-model="open" :title="title" width="600px" append-to-body>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-row>
          ...
          <el-col v-if="form.taskType == 2" :lg="24">
          ...
          <el-col v-if="form.triggerType == 1" :lg="24">
    ...
    <el-dialog v-model="openCron" title="Cron表达式生成器" destroy-on-close>
      <!-- 使用组件例子 -->
      <Vue3CronCore v-model:value="form.cron" i18n="cn" max-height="350px" style="flex: 0.4" @change="changeCron" />
    </el-dialog>

    <el-drawer v-model="drawer" :title="logTitle">
      <el-form :inline="true" @submit.prevent>
        <el-form-item>
          <el-date-picker v-model="logForm.beginTime" clearable type="date" />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" icon="search" @click="handleJobLog">
            {{ $t('btn.search') }}
          </el-button>
        </el-form-item>
      </el-form>
      ...
      <el-timeline>
        <el-timeline-item v-for="(item, i) in jobLogList" :key="i" :timestamp="item.createTime" placement="top">
          <h4>{{ item.jobMessage }}</h4>
          <p>{{ item.exception }}</p>
        </el-timeline-item>
      </el-timeline>
      <el-empty v-if="jobLogList.length <= 0" />
    </el-drawer>
  </div>
</template>
