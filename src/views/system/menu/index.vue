...
import { addMenu, changeMenuSort as changeSort, delMenu, getMenu, listMenu, listMenuById, updateMenu } from '@/api/system/menu'
import IconSelect from '@/components/IconSelect'
import SvgIcon from '@/components/SvgIcon'

const { proxy } = getCurrentInstance()

const dictParams = [{ dictType: 'sys_show_hide' }, { dictType: 'sys_normal_disable' }]
const menuList = ref([])
  ...
  <div class="app-container">
    <el-form v-show="showSearch" ref="queryRef" :model="queryParams" :inline="true">
      <el-form-item :label="$t('m.parentMenu')" prop="parentId">
        <el-cascader
          v-model="queryParams.parentId" class="w100" :options="menuQueryOptions"
      ...
      <el-form-item :label="$t('m.menuState')" prop="status">
        <el-select v-model="queryParams.status" placeholder="菜单状态" clearable>
          <el-option
            v-for="dict in sys_normal_disable" :key="dict.dictValue" :label="dict.dictLabel"
      ...
      <el-form-item :label="$t('m.isShow')" prop="visible">
        <el-select v-model="queryParams.visible" placeholder="显示状态" clearable>
          <el-option
            v-for="dict in sys_show_hide" :key="dict.dictValue" :label="dict.dictLabel"
            :value="dict.dictValue"
          />
        </el-select>
      </el-form-item>
    ...
    <el-table
      v-if="refreshTable" ref="listRef" v-loading="loading" :data="menuList" row-key="menuId"
      :default-expand-all="isExpandAll" border lazy :load="loadMenu"
    ...
    >
      ...
      <el-table-column prop="menuId" :label="$t('m.menuid')" :show-overflow-tooltip="true" width="80" align="center" />
      <el-table-column prop="menuType" :label="$t('m.menuType')" align="center" width="80">
        <template #default="scope">
          <el-tag v-if="scope.row.menuType == 'M' && scope.row.isFrame == 1" type="danger">
            {{ $t('m.link') }}
          </el-tag>
          <el-tag v-else-if="scope.row.menuType == 'C'">
            {{ $t('m.menu') }}
          </el-tag>
          <el-tag v-else-if="scope.row.menuType == 'M'" type="success">
            {{ $t('m.directory') }}
          </el-tag>
          <el-tag v-else-if="scope.row.menuType == 'F'" type="warning">
            {{ $t('m.button') }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="orderNum" :label="$t('m.sort')" width="90" sortable align="center">
        <template #default="scope">
          ...
          <el-input
            v-show="editIndex == scope.$index" :ref="setColumnsRef" v-model="scope.row.orderNum"
            @blur="handleChangeSort(scope.row)"
          />
        </template>
      </el-table-column>
      <el-table-column prop="perms" :label="$t('m.authorityID')" :show-overflow-tooltip="true" />
      <el-table-column prop="component" :label="$t('m.componentPath')" :show-overflow-tooltip="true" />
      ...
      <el-table-column :label="$t('btn.operate')" align="center" width="170" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button v-hasPermi="['system:menu:edit']" text size="small" icon="Edit" @click="handleUpdate(scope.row)" />
          <el-button v-hasPermi="['system:menu:add']" text size="small" icon="Plus" @click="handleAdd(scope.row)" />
          <el-button
            v-hasPermi="['system:menu:remove']" text size="small" icon="Delete"
        ...
        <el-row>
          <el-col :lg="24">
            <el-form-item :label="$t('m.parentMenu')">
              <el-cascader
                v-model="form.parentId" class="w100" :options="menuOptions"
          ...
          <el-col v-if="form.menuType != 'F'" :lg="24">
            <el-form-item :label="$t('m.icon')" prop="icon">
              <el-popover
                v-model:visible="showChooseIcon" placement="bottom-start" :teleported="false" :width="540"
                trigger="click"
              >
                <template #reference>
          ...
          <el-col v-if="form.menuType != 'F'" :lg="12">
            <el-form-item>
              <template #label>
                <span>
          ...
          <el-col v-if="form.menuType == 'C'" :lg="12">
          ...
          <el-col v-if="form.menuType != 'M'" :lg="12">
            <el-form-item>
          ...
          <el-col v-if="form.menuType != 'F'" :lg="12">
            <el-form-item prop="visible">
              ...
              <el-radio-group v-model="form.visible">
                <el-radio v-for="dict in sys_show_hide" :key="dict.dictValue" :label="dict.dictValue">
                  {{ dict.dictLabel }}
                </el-radio>
              </el-radio-group>
            </el-form-item>
